...
Identify the primary deliverable and the minimum decisive evidence before starting tools. Finish the requested report, review finding, comment draft, or code change before optional diagrams, exports, formatting polish, broad scans, or redundant validation.

For read-only code review, prioritize the current diff, complete discussion, affected call paths, reachable counterexamples, and the smallest check that can confirm or falsify a finding. Do not run broad package test matrices merely to make a source-level review look more complete; run them only when the patch changes executable code under review or when a conclusion genuinely depends on runtime behavior.

Do not start duplicate long-running builds, tests, browser sessions, or renderers for the same question. Before a potentially expensive command, state which unresolved decision its result will change. If an optional supporting tool fails once, stop spending the work loop on it unless that artifact is itself the requested deliverable.
...
For technical-route HTML presentations, default to a fixed `1920×1080` stage that scales as one 16:9 canvas without mobile reflow, and keep navigation chrome from consuming slide height. Preserve two useful visual directions: Swiss Modern / Style A for speaker-led, table-free narrative slides with strong geometry and red, blue, and green accents on paper; use orange only as a sparse validation or transition accent when a multi-step flow needs another semantic color, and avoid yellow accents on its white background. Use Signal / Style C for high-density evidence with navy, warm paper, antique-gold emphasis, hairline structure, and an explicit cross-slide narrative bridge. For recurring domain nouns in a live technical deck, introduce the Chinese term with its English reference on the first key slide, such as `副本（replica）` and `工作负载（workload）`, then use the concise Chinese term afterward. Expand unfamiliar abbreviations on first use and add their plain Chinese purpose, for example `GVK（Group / Version / Kind，资源类型标识）`. When creating an alternate style, keep the existing version unless the user explicitly asks to replace it.

Use `.agents/skills/build-technical-research-slides/` when creating or revising a technical research HTML deck, including narrative compression, terminology cleanup, Style A implementation, browser validation, and replacement of a superseded version.

For Chinese terminology and prose cleanup, the local Humanizer-cs skill lives at `/home/humanizer-cs/skills/humanizer-cs/SKILL.md`. Read that entry and its required references before editing, and protect exact code identifiers, API fields, modes, plugin names, issue numbers, and evidence boundaries from free-form rewriting.

On the current Linux workspace, reuse the installed Playwright `1.62.1` browser cache before downloading another browser. The existing full Chromium is `/root/.cache/ms-playwright/chromium-1234/chrome-linux64/chrome`, and the headless shell is `/root/.cache/ms-playwright/chromium_headless_shell-1234/chrome-headless-shell-linux64/chrome-headless-shell`. Do not pin an older Playwright version that requests a different browser build unless the existing browser is proven incompatible.
...
For a one-line maintainer suggestion (such as a helper-name or wording change), use one natural sentence that records the change. Do not use a generic status sentence combining a commit SHA with a test summary, and do not append SHA/test details unless the reviewer asked for validation evidence.
...
For local technical research and code-review records, the canonical editable diagram source is sufficient by default; a Mermaid `.mmd` does not require a PNG/SVG companion. Export only when the user explicitly requests it, an HTML/presentation must embed it, an upstream/community artifact needs a directly viewable image, or visual inspection is necessary to validate the deliverable. Rendering is supporting work and must not delay the report, findings, or review comment; reuse the existing browser and stop after one failed optional attempt.
