...
当前结论仍是 E3。官方失败和独立 fork 失败提供了真实 CI 日志、源码顺序和 producer-to-impact 时间线，
但本地没有 Karmada 多集群环境，也没有在同一受控环境完成“补丁前稳定失败、补丁后稳定通过”的
counterfactual。PR 已提交，upstream 的三组 E2E 在本次记录时仍在运行，不能写成已根治或已被接受。
...
spec 创建了 `poddisruptionbudget-xw489` Deployment，却只注册了 PDB cleanup。该 Deployment 随后继续
出现在 estimator assumption input，直到五分钟 TTL 附近消失，并形成相同的“第一次 no-fit、随后 probe
fit”终点。

这条 fork 证据没有制造官方失败里的 `deploy-wbch9`。两者是两个独立的资源泄漏点，共享同一个 E2E
隔离合同：会创建 workload 的 spec 必须清除 source object 和生成的 ResourceBinding，不能让它们进入后续
estimator spec。
...
- 截至 2026-08-13 10:11 CST，DCO、codegen、compile、lint、unit test 和三组普通 Kubernetes test
  已通过；`e2e test (v1.34.0/v1.35.0/v1.36.1)` 仍为 pending。
- Tide 仍等待 `lgtm` 和 `approved`。没有主动请求 reviewer，也没有发布额外 comment 或触发 retest。
...
3. 修复覆盖 taint/toleration 和 PDB 两个已确认 producer。其他 E2E spec 若遗漏 workload cleanup，仍可能
   形成新的跨用例污染，需要新的日志链确认后分别修复。
