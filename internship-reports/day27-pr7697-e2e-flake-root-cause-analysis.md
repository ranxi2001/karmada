...
2. 两个候选均已在独立 fork branch 完成最小实现：Remedy 两文件修复达到局部 E4；migration 一文件同步屏障完成 package compile；两条 fork branch 的 v1.34/v1.35/v1.36 E2E matrix 均通过。
...
6. 同 SHA `/retest` 转绿只证明 manifestation 具有非确定性，不撤销 Remedy 的 E3 根因，也不构成继续等待的理由。修复必须作为独立 issue/PR 推进，不得混入证书 PR #7697。
...
| --- | --- | --- | --- | --- | --- |
| P0 | Remedy action cleanup 不收敛 | 历史 #5323；Day 11、#7770、#7697 三条新样本 | E4：旧 predicate 下 regression 失败，修复后通过；三版本 fork E2E 通过 | `FORK_CI_GREEN` | 准备 Remedy-specific issue/PR exact text，获取用户确认后再发布 |
...
| HOLD | control-plane / etcd / aggregated API collapse | 本窗口 16 jobs，多 PR、多 Kubernetes matrix；包含 #7723 restart-check | collapse 链可到 E3；共同 host trigger 仅 E2 | `NEEDS_RCA` | 先取得 runner host I/O、pressure、container exit 与磁盘证据，再决定 CI harness 还是环境修复 |
...
当前没有创建 upstream issue/PR，也没有发布评论。发布前仍需分别准备 exact target 与英文文本并让用户确认。
...
- `2026-07-17` 检索 open issue/PR，没有发现包含 `RemedyActions`、`remedy controller`、`#5323`、`#6858` 或 `#6899` 的活跃修复。

因此无需再等待 same-SHA retest 第四次命中。应把 #5323、Day 11、#7770 和 #7697 的连续样本整理进新的 Remedy-specific issue，或先询问维护者是否 reopen #5323；随后准备独立 patch，不恢复 #7077 的全局 helper 改造。
...
本周完成一项已合并 flake 修复闭环：#7732 修复 FlinkDeployment cleanup/APIEnablements 竞态并关闭 #7719。Day 11 之后的 83 个 upstream PR `CI Workflow` runs 中，严格排除代码错误后有 23 runs、29 jobs 被归为 flake。候选筛选后有两项值得开独立 PR：Remedy status cleanup 已跨窗口出现三条新样本，#7697 日志与源码达到 E3，最小方案是让 `RemedyActions`-only update 触发补偿 reconcile 并补 focused regression；migration e2e 则应使用现有 `WaitResourceBindingFitWith` 等待其实际断言的 Applied + Healthy 状态。其余 25 jobs 仍需 RCA，不能用 timeout 或通用 retry 伪装修复。#7723 restart artifact 已归入 control-plane collapse：etcd readiness 失败导致 lease renewal timeout 和正常的 `leader election lost` 退出，不是独立 panic 候选。
