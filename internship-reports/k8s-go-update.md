...
- 当前边界：本地 topic branch 已实现和验证；未经确认不 push、不创建 upstream PR。
...
- push、PR title/body 和任何 upstream action 留到用户确认之后。

## 实现结果

- work-api branch：`deps/kubernetes-1.36-go-1.26`
- base：`upstream/master@9710f2f9d7c6359c76d501104df86e1278942772`
- 本地 commit：`cf1bfed` `Bump Kubernetes dependencies to v1.36.4`
- commit 状态：包含 `Signed-off-by: ranxi2001 <ranxi169@163.com>`；未 push、未创建 PR。
...
`k8s.io/apiextensions-apiserver`、`k8s.io/apiserver`、`k8s.io/component-base` 等 tool-only transitive modules 由 controller-tools/controller-runtime 解析为 `v0.36.0`。direct runtime/codegen modules 使用任务要求的最新 `v0.36.4`。这与 #49 中 direct patch 与 transitive tool patch 不完全一致的处理方式相同，没有人为强制所有间接模块到 `.4`。
...
第一次运行 `go test ./...` 时，所有 package 已完成编译，但 test execution 失败：controller suite 找不到 `/usr/local/kubebuilder/bin/etcd`，E2E suite 没有 kubeconfig。后续使用仓库标准 `make test` 下载 envtest assets 并仅运行 `pkg/...`，测试通过。live E2E 没有运行。
...
依赖升级可以在不修改 hand-written API/controller 逻辑的情况下完成，标准 build、unit、verify 与 lint 均通过。需要在 PR body 或 reviewer notes 中主动披露 generated `SharedInformerFactory` 的 source-incompatible method additions；不能只写“依赖升级，无接口影响”。

下一步在任何 push 前，先准备 concise PR body，说明版本选择、生成物、interface compatibility 和未运行 live E2E，并让用户确认 exact branch/title/body。
