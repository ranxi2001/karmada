...
| --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | 9 月前拿到 AgentCube Karmada 项目社区席位 | DOING | 高 | 中 | 7-8 周 | upstream issue/PR/review/CI/会议材料证据链；每周 mentor 可检查总结 | Week 33 以 #7492 为核心；先按 maintainer-provided Draft 补齐 PR1 或明确更窄切分，再准备 exact PR text 并进入 PR2/PR3 行为实现；#7621/#7662 保持战略 review 线，#7697 保持交付维护线 |
| P0 | 实现 #7492 multi-component scale rescheduling | DOING | 高 | 高 | 1 周起 | 已 `/assign @ranxi2001`；[maintainer API 回复](https://github.com/karmada-io/karmada/issues/7492#issuecomment-5248383498)；[maintainer-provided Draft](https://github.com/RainbowMango/karmada/blob/c14af2f1119a66d4672a814cc80f7612943d35d3/docs/proposals/scheduling/multi-podtemplate-support/scheduling-result-for-components.md)；[Day 44 已固定答案](day44-issue7492-component-scheduling-result-api-design.md)；[Day 45 完整性分析](day45-issue7492-progress-completeness-and-open-contracts.md)：feature `a3547a3a8` 为 0 behind/1 ahead、单 DCO commit，`make test`/`make verify` 通过；完整 feature 约 25%，无关联 PR | 先补 Draft PR1 的 `GracefulEvictionTask.Components` 与 RB result validation/tests，或明确获得更窄 API-only split 的认可，再准备 exact title/body；并行闭合 v1alpha1 main/status、failed-rescheduling safety 和 scale delta/trigger |
