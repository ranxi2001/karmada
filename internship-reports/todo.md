...
| P0 | 准备下周一 Karmada Descheduler 专项汇报 | REVIEW | 高 | 中 | 0.5 天 | [Day 38 专项报告](day38-karmada-descheduler-special-study.md)；[13 页 HTML 汇报稿](day38-karmada-descheduler-presentation.html)；[控制闭环 PNG](day38-karmada-descheduler-control-loop.png)；[Karmada/Kubernetes 对比 PNG](day38-karmada-vs-kubernetes-descheduler.png)；Karmada `ce2a7b8` 与 Kubernetes Descheduler `7d2b28b` 源码证据 | 用 HTML 汇报稿试讲并压到 15 分钟；向团队确认千问 workload 的真实 Kind、是否多组件/可替换，以及成功条件是 Binding 重分配还是 target Ready |
| P0 | 跟进 #7621 / proposal PR #7662 复杂工作负载安全重调度 | DOING | 高 | 高 | proposal 合同收敛后再估算实现 | [Day 15 尽调](day15-issue-7621-safe-rescheduling-feature.md)；[Day 29 #5070 / Full affinity 语义](day29-issue5070-pr7662-fresh-rescheduling-research.md)；[Day 30 maintainer API 方向](day30-pr7662-maintainer-api-direction.md)；[Day 31 API/开发方案](day31-workload-rebalancer-api-development-plan.md)；[Day 35 unavailable/Pending/workload 矩阵](day35-pr7662-workload-unavailable-pending-research.md)；[Day 37 selector / unschedulable 更新 review](day37-pr7662-selector-unschedulable-review.md)；[Day 38 Descheduler 专项](day38-karmada-descheduler-special-study.md)；[target-first review](https://github.com/karmada-io/karmada/pull/7662#discussion_r3576720182) | typed API 暂停。7 月 28 日会议仅收敛 offline workload scope；7 月 30 日 maintainer selector 方案能看见 assigned-but-unschedulable Pod，但仍需统一 Available/Unschedulable API 语义，定义 selector ownership/rollout、多组件到标量 deficit 的映射、`MultiplePodTemplatesScheduling` gate、scheduler/Descheduler single writer 和 completion contract。等待 `@zhy76` / `@RainbowMango` 回复或 proposal commit |
| P0 | 跟进 #5070 / #5172 multiple affinity Fresh reset | REVIEW | 高 | 中 | 等待 upstream CI / human review | [Day 29 调研](day29-issue5070-pr7662-fresh-rescheduling-research.md)；[Day 31 历史实现](day31-workload-rebalancer-api-development-plan.md)；[Day 34 mentor scope 纠偏、clean rebuild、发布与 rebase](day34-pr7791-mentor-scope-correction.md)；[#5425](https://github.com/karmada-io/karmada/pull/5425) 已合并 6 行实现；[PR #7791](https://github.com/karmada-io/karmada/pull/7791) 已按 maintainer 要求 rebase，current head `11030fbe1` 仅含 2 个测试文件 `+288/-13`，`range-diff` 证明测试 patch 不变，DCO/sign-off 和 `diff --check` 通过；按用户要求未做 post-rebase test rerun | 等待 current-head upstream checks 和 maintainer re-LGTM/approve；force-push 已清除旧 `lgtm`，不主动 `/retest`、催审或发标签命令；继续守住 scheduler cache ownership 边界 |
