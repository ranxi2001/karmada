...
| --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | 9 月前拿到 AgentCube Karmada 项目社区席位 | DOING | 高 | 中 | 7-8 周 | upstream issue/PR/review/CI/会议材料证据链；每周 mentor 可检查总结 | Week 33 以 #7492 为核心；Day 44 已完成 API 分支 preflight，先闭合 admission/版本合同再进入调度行为；#7621/#7662 保持战略 review 线，#7697 保持交付维护线 |
| P0 | 实现 #7492 multi-component scale rescheduling | DOING | 高 | 高 | 1 周起 | 已 `/assign @ranxi2001`；[maintainer API 回复](https://github.com/karmada-io/karmada/issues/7492#issuecomment-5248383498)；[Day 44 API 与分支实测](day44-issue7492-component-scheduling-result-api-design.md)：个人分支 `b0501d9b` 已加入结果类型/生成物/legacy projection，`make test`/`make verify` 通过；11 项 CRD 矩阵中 4 项错误接受，helper 判等顺序敏感；`ReviseComponents`、Gate/version admission 和正文三项行为未实现；无关联 PR | 先把 Day 44 矩阵落成正式 admission/version tests，闭合 partial/unknown/scalar/requiredBy、Gate、v1alpha1 update 和 name-keyed equality，再进入 scheduler producer/consumer；旧协调评论不发 |
