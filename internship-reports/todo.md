...
| P1 | 跟进 estimator / FlinkDeployment / ResourceQuota e2e flake | REVIEW | 中 | 低 | 0.5-1 天 | [Upstream issue #7719](https://github.com/karmada-io/karmada/issues/7719)；[Upstream PR #7732](https://github.com/karmada-io/karmada/pull/7732)；[Day 11 设计与代码复查](day11-ci-flake-statistics.md#7719-修复-pr-7732)；`kind/flake` label 已生效；干净分支 `test/flinkdeployment-crd-cleanup` commit `1240559dd`；本地 focused `go test` 通过；fork CI 与 upstream lint/codegen/compile/unit/e2e v1.34-v1.36 通过；代码复查无 correctness finding；upstream Chart v1.34 和 Operator v1.35 各有一个 cancelled job | 等 human reviewer 的 `lgtm`/approval；如 maintainer 要求全绿，经用户确认后请求 rerun cancelled jobs，不因无关取消修改代码 |
