...
| P0 | 准备下周一 Karmada Descheduler 专项汇报 | REVIEW | 高 | 中 | 0.5 天 | [Day 38 广度报告](day38-karmada-descheduler-special-study.md)；[Day 39 五层代码合同与四方案对比](day39-karmada-descheduler-code-contracts-and-options.md)；[16 页 Day 39 Style A 汇报稿](day39-karmada-descheduler-code-research-presentation.html)；[代码链与五层断点 PNG](day39-karmada-descheduler-code-contract-breaks.png)；Karmada `a5cf21eac` 定向单测与浏览器验收通过 | 用 Day 39 HTML 试讲；拿千问真实 YAML 逐层确认 GVK、owner、progress、placement、completion，判断走 B 的可替换标量 adapter，还是先确认执行方后走 D；只有逐组件跨集群硬需求才启动 C |
| P0 | 跟进 #7621 / proposal PR #7662 复杂工作负载安全重调度 | DOING | 高 | 高 | proposal 合同收敛后再估算实现 | [Day 15 尽调](day15-issue-7621-safe-rescheduling-feature.md)；[Day 29 #5070 / Full affinity 语义](day29-issue5070-pr7662-fresh-rescheduling-research.md)；[Day 30 maintainer API 方向](day30-pr7662-maintainer-api-direction.md)；[Day 31 API/开发方案](day31-workload-rebalancer-api-development-plan.md)；[Day 35 unavailable/Pending/workload 矩阵](day35-pr7662-workload-unavailable-pending-research.md)；[Day 37 selector / unschedulable 更新 review](day37-pr7662-selector-unschedulable-review.md)；[Day 38 Descheduler 专项](day38-karmada-descheduler-special-study.md)；[Day 39 五层代码合同与方案](day39-karmada-descheduler-code-contracts-and-options.md)；[target-first review](https://github.com/karmada-io/karmada/pull/7662#discussion_r3576720182) | typed API 暂停。除 Available/Unschedulable、selector ownership/rollout、多组件映射、feature gate 与 single writer 外，还需逐层收敛 capability、Pod owner、progress、placement 和 completion；复杂类型先比较扩展 embedded `GracefulEvictionTasks` 与独立 task，并确认 adapter/Operator/storage plane 执行方。等待 `@zhy76` / `@RainbowMango` 回复或 proposal commit |
