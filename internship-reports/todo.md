...
| --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | 9 月前拿到 AgentCube Karmada 项目社区席位 | DOING | 高 | 中 | 7-8 周 | upstream issue/PR/review/CI/会议材料证据链；每周 mentor 可检查总结 | Week 33 以 #7492 为核心；Draft PR1 branch candidate 已完成并推送，准备 exact PR text 后进入 PR2/PR3 行为实现；#7621/#7662 保持战略 review 线，#7697 保持交付维护线 |
| P0 | 实现 #7492 multi-component scale rescheduling | DOING | 高 | 高 | 1 周起 | 已 `/assign @ranxi2001`；[maintainer API 回复](https://github.com/karmada-io/karmada/issues/7492#issuecomment-5248383498)；[maintainer-provided Draft](https://github.com/RainbowMango/karmada/blob/c14af2f1119a66d4672a814cc80f7612943d35d3/docs/proposals/scheduling/multi-podtemplate-support/scheduling-result-for-components.md)；[Day 45 完整性分析](day45-issue7492-progress-completeness-and-open-contracts.md)：fork branch `cf59527e2` 为 0 behind/1 ahead、单 DCO commit，完整 feature 约 30%；[Day 46 状态问题复现](day46-issue7492-mszacillo-state-reproduction.md)：精确 Flink 场景未复现，但源码级条件机制已验证，纯 `Components` 扩容不触发调度，第二触发器换集群时普通路径不构造 `PreservedLabelState` | 当前不发布重复性 comment；等待 `mszacillo` 补版本、YAML、第二触发器和 state 定义，再决定是否追加一条只包含新证据的短回复；PR2 前明确跨集群状态合同 |
