...
| --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | 9 月前拿到 AgentCube Karmada 项目社区席位 | DOING | 高 | 中 | 7-8 周 | upstream issue/PR/review/CI/会议材料证据链；每周 mentor 可检查总结 | Week 33 以 #7492 为核心；Draft PR1 branch candidate 已完成并推送，先闭合 legacy-write 数据保护和 live regression，再准备 exact PR text；#7621/#7662 保持战略 review 线，#7697 保持交付维护线 |
| P0 | 实现 #7492 multi-component scale rescheduling | DOING | 高 | 高 | 1 周起 | 已 `/assign @ranxi2001`；[maintainer API 回复](https://github.com/karmada-io/karmada/issues/7492#issuecomment-5248383498)；[maintainer-provided Draft](https://github.com/RainbowMango/karmada/blob/c14af2f1119a66d4672a814cc80f7612943d35d3/docs/proposals/scheduling/multi-podtemplate-support/scheduling-result-for-components.md)；PR1 head `1382c90d9` exact-SHA CI 全绿；[Day 47 修复设计](day47-issue7492-v1alpha1-status-data-loss-fix-design.md)确认 `v1alpha1 /status` 可擦除 component data，当前 webhook rule 又不匹配 subresource | 先补 A2 exact status rules、storage-state guard、status early return 和 RB/CRB live regression；rebase 后重验再准备 PR。A2 只保护 component state，完整 legacy conversion 或停止 serving 另行讨论；PR2 前再明确跨集群状态合同 |
...
| P1 | 跟进 EstimatorAssumption 跨 spec 清理 flake | REVIEW | 中 | 低 | 等待 upstream | [Issue #7826](https://github.com/karmada-io/karmada/issues/7826)；test-only [PR #7827](https://github.com/karmada-io/karmada/pull/7827) 为单 DCO commit `ba531a9a1`、3 文件 `+53/-23`；[Day 48 E3 RCA 与修复边界](day48-estimator-assumption-e2e-isolation-pr7827.md)；DCO/codegen/compile/lint/unit 和普通 Kubernetes tests 已通过 | 等三组 upstream E2E 与 maintainer review；保持 E3，不把单次绿灯写成 E4。只在 current SHA 失败或 review 出现新信号时定向处理，不主动 retest/comment |
