...
| P0 | 实现 #7492 multi-component scale rescheduling | DOING | 高 | 高 | 1 周起 | 已 `/assign @ranxi2001`；[maintainer API 回复](https://github.com/karmada-io/karmada/issues/7492#issuecomment-5248383498)；[Day 44 API、修复与 maintainer 问题清单](day44-issue7492-component-scheduling-result-api-design.md)：feature 已 squash 并推送为单个 DCO commit `a3547a3a8`，tree 与 squash 前一致；map-list 判等、11 个用例和中性注释已完成，`make test`/`make verify` 通过，仍无 producer/consumer 或关联 PR | 先确认 Day 44 的 9 个 maintainer 问题，再落 admission/version tests 并进入 scheduler producer/consumer；旧协调评论不发 |
