...
| --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | 9 月前拿到 AgentCube Karmada 项目社区席位 | DOING | 高 | 中 | 7-8 周 | upstream issue/PR/review/CI/会议材料证据链；每周 mentor 可检查总结 | Week 33 以 #7492 为核心，先等详细 API 并做 Day 43 对照；#7621/#7662 保持战略 review 线，#7697 保持交付维护线 |
| P0 | 实现 #7492 multi-component scale rescheduling | BLOCKED | 高 | 高 | 1 周起 | 已 `/assign @ranxi2001`；[Day 42 上下文基线与详细 API 等待状态](day42-issue7492-multi-component-scale-rescheduling-intake.md)；[Day 43 #5115 演进与快速实现路线](day43-issue5115-evolution-and-7492-implementation-plan.md)；`upstream/master@c884a959`；无关联 PR | 等 `RainbowMango` 给出详细 API；固定官方 ref 后对照 result/conversion/delta/failure/legacy/FRQ/RB-CRB 矩阵，对照前不建 worktree、不发旧评论 |
...
| P1 | 建立 Karmada CI flake 专项台账 | REVIEW | 中 | 低 | 0.5-1 天 | [Day 11 固定窗口统计](day11-ci-flake-statistics.md)；[Day 27 可修复候选、实现与 fork CI](day27-pr7697-e2e-flake-root-cause-analysis.md)；[Day 33 #7791 新样本与 `karmadactl top` fixture/terminal evidence split](day33-pr7791-e2e-flake-root-cause-analysis.md)；[#7795 与 CI 深入 RCA](day33-karmadactl-top-flake-upstream-draft.md#upstream-ci-红项分类2026-08-10)；Remedy [issue #7776](https://github.com/karmada-io/karmada/issues/7776) 已关闭，[PR #7777](https://github.com/karmada-io/karmada/pull/7777) 已合并为 `eb2e7c75ff82` | #7795 fixture 修复为 E4；当前红项终止于 kind cgroup-ready 失败，底层共享 runner I/O/运行时抖动假设为 E2。`/retest` 被 bot 拒绝且不能重跑 GHA，等待维护者执行 **Re-run failed jobs** 后验证同 SHA v1.35 非确定性；继续决定 migration `8ef8a08c6` 是否独立发布 |
