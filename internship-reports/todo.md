...
| P1 | 建立 Karmada CI flake 专项台账 | REVIEW | 中 | 低 | 0.5-1 天 | [Day 11 固定窗口统计](day11-ci-flake-statistics.md)；[Day 27 可修复候选、实现与 fork CI](day27-pr7697-e2e-flake-root-cause-analysis.md)；Remedy [issue #7776](https://github.com/karmada-io/karmada/issues/7776) / [PR #7777](https://github.com/karmada-io/karmada/pull/7777) 当前为单提交 `dcd150b17`，局部 E4、终态 no-op 反向测试与普通/race 包测试通过；migration `8ef8a08c6` exact-state wait 待单独发布决策 | 跟进 #7777 新 head upstream CI，转绿后请求 human review；migration 仍需独立 exact-text 确认；其余 25 jobs 继续补 RCA，不开 timeout/retry 型 PR |
