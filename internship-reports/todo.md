...
| P0 | 跟进 #7621 / proposal PR #7662 复杂工作负载安全重调度 | DOING | 高 | 高 | 分两个可交付行为 PR 推进 | [Day 15 尽调](day15-issue-7621-safe-rescheduling-feature.md)；[Day 29 #5070 / Full affinity 语义](day29-issue5070-pr7662-fresh-rescheduling-research.md)；[Day 30 maintainer API 方向](day30-pr7662-maintainer-api-direction.md)；[Day 31 API/开发方案](day31-workload-rebalancer-api-development-plan.md)；[target-first review](https://github.com/karmada-io/karmada/pull/7662#discussion_r3576720182) | 不再等 proposal author 先改稿。先完成不改 API 的 #5070 Full cursor reset；随后以单独端到端 PR 落 typed request + Deployment Aggregated/Dynamic preserve。SafeMigration、自动水位和 #7621 closing claim 继续排除 |
| P0 | 跟进 #5070 / #5172 multiple affinity Fresh reset | DOING | 高 | 中 | 1-2 天 | [Day 29 调研](day29-issue5070-pr7662-fresh-rescheduling-research.md)；[Day 31 实现方案](day31-workload-rebalancer-api-development-plan.md)；[#5070 `/assign`](https://github.com/karmada-io/karmada/issues/5070#issuecomment-5022199800)，current assignee `@ranxi2001`；#5172 仍 assign `@bharathguvvala`；#5425 stale/blocked | 从最新 upstream/master 建 clean `feature/reset-affinity-on-reschedule`：只改 scheduler RB/CRB affinity 起点、对应 UT 和真实 A -> B -> A E2E；本地与 fork push CI 通过后准备 clean-replacement comment/PR exact text，不接管 #5172 ownership |
...
| P1 | 建立 Karmada CI flake 专项台账 | REVIEW | 中 | 低 | 0.5-1 天 | [Day 11 固定窗口统计](day11-ci-flake-statistics.md)；[Day 27 可修复候选、实现与 fork CI](day27-pr7697-e2e-flake-root-cause-analysis.md)；Remedy [issue #7776](https://github.com/karmada-io/karmada/issues/7776) / [PR #7777](https://github.com/karmada-io/karmada/pull/7777) 当前为单提交 `dcd150b17`；维护者已基本确认真实问题及 #6858 同类根因，并明确接受 caller-specific trigger 的 no-op reconcile 取舍；17 个 check runs 全绿 | #7777 当前无代码修改要求，等待正式 `lgtm`/`approved`；`COMMENTED` 方案接受不能写成已批准。migration 仍需独立 exact-text 确认；其余未闭环 jobs 继续补 RCA，不开 timeout/retry 型 PR |
