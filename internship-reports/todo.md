...
| --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | 9 月前拿到 AgentCube Karmada 项目社区席位 | DOING | 高 | 中 | 7-8 周 | upstream issue/PR/review/CI/会议材料证据链；每周 mentor 可检查总结 | Week 33 以 #7492 为核心；Day 45 已确认 API-groundwork branch 可进入 PR review，先准备 exact text，再闭合 merge 合同并进入调度行为；#7621/#7662 保持战略 review 线，#7697 保持交付维护线 |
| P0 | 实现 #7492 multi-component scale rescheduling | DOING | 高 | 高 | 1 周起 | 已 `/assign @ranxi2001`；[maintainer API 回复](https://github.com/karmada-io/karmada/issues/7492#issuecomment-5248383498)；[Day 44 API 设计与分支实测](day44-issue7492-component-scheduling-result-api-design.md)；[Day 45 进度、完整性与三组 merge blocker](day45-issue7492-progress-completeness-and-open-contracts.md)：feature `a3547a3a8` 为 0 behind/1 ahead、单 DCO commit，`make test`/`make verify` 通过；groundwork PR readiness 约 85%–90%，完整 feature 约 25%，无关联 PR | 准备 exact title/body，经用户确认后开 `Part of #7492` 的 API-groundwork PR；merge 前确认结果合同、`BindingSnapshot` surface 和 v1alpha1 数据完整性，再实现 producer/trigger/dispatch/failure safety |
