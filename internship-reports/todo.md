...
| P1 | 建立 Karmada CI flake 专项台账 | REVIEW | 中 | 低 | 0.5-1 天 | [Day 11 固定窗口统计](day11-ci-flake-statistics.md)；[Day 27 可修复候选、实现与 fork CI](day27-pr7697-e2e-flake-root-cause-analysis.md)；Remedy `3861906f2` 局部 E4，migration `8ef8a08c6` exact-state wait；两条分支三版本 E2E 全绿 | 两项进入 upstream exact-text 准备门禁，发布前分别确认 target/text；其余 25 jobs 继续补 RCA，不开 timeout/retry 型 PR |
