...
| P0 | 实现 #7492 multi-component scale rescheduling | DOING | 高 | 高 | 1 周起 | 已 `/assign @ranxi2001`；[maintainer API 回复](https://github.com/karmada-io/karmada/issues/7492#issuecomment-5248383498)；[Day 44 API、修复与 maintainer 问题清单](day44-issue7492-component-scheduling-result-api-design.md)：feature head `5f0ae9b24`，2 个本地 DCO commit 已修复 map-list 判等并将 “only multiple” 收敛为中性注释，`make test`/`make verify` 通过，未推送；仍无 producer/consumer 或关联 PR | 先确认 Day 44 的 9 个 maintainer 问题，再落 admission/version tests 并进入 scheduler producer/consumer；旧协调评论不发 |
