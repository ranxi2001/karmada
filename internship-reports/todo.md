...
| --- | --- | --- | --- | --- | --- | --- | --- |
| P0 | 9 月前拿到 AgentCube Karmada 项目社区席位 | DOING | 高 | 中 | 7-8 周 | upstream issue/PR/review/CI/会议材料证据链；每周 mentor 可检查总结 | Week 33 以 [#7492 PR1 #7830](https://github.com/karmada-io/karmada/pull/7830) review 为核心；current-SHA upstream CI 全绿，先修复已确认的 nested component validation P1，再等待 human review；#7621/#7662 保持战略 review 线，#7697 保持交付维护线 |
| P0 | 实现 #7492 multi-component scale rescheduling | DOING | 高 | 高 | 1 周起 | 已 `/assign @ranxi2001`；[maintainer API 回复](https://github.com/karmada-io/karmada/issues/7492#issuecomment-5248383498)；[maintainer-provided Draft](https://github.com/RainbowMango/karmada/blob/c14af2f1119a66d4672a814cc80f7612943d35d3/docs/proposals/scheduling/multi-podtemplate-support/scheduling-result-for-components.md)；PR1 [#7830](https://github.com/karmada-io/karmada/pull/7830) head `be8c7c3f7` 为单 DCO commit、25 文件；17/17 upstream checks 全绿；[Day 49](day49-issue7492-pr1-api-compat-pr7830.md)记录 exact CI 与 review 状态 | 在现有 `validating.go`/`validating_test.go` 补 ownership-aware nested result validation：两条路径都做 gate-off 新旧值保护，eviction 按当前 desired 校验 membership，`RequiredBy` 只校验自身 duplicate。完成 focused tests/`make verify` 后再申请开放 PR branch push 与回复授权；PR1 稳定后再推进 PR2 |
...
| P1 | 跟进 EstimatorAssumption 跨 spec 清理 flake | REVIEW | 中 | 低 | 0.5-1 天 | [Issue #7826](https://github.com/karmada-io/karmada/issues/7826)；test-only [PR #7827](https://github.com/karmada-io/karmada/pull/7827) head 仍为 `ba531a9a1`；[专用集群评论](https://github.com/karmada-io/karmada/pull/7827#issuecomment-5291634887)；[Day 48 E3 RCA 与单文件设计](day48-estimator-assumption-e2e-isolation-pr7827.md) | 用户决定实施后，只改 `estimator_test.go`，复用动态 Kind 先例并部署独立 estimator；PDB cleanup 移出主修复。开放 PR branch/body 更新前重新确认 exact action |
| P1 | 建立 Karmada CI flake 专项台账 | REVIEW | 中 | 低 | 0.5-1 天 | [Day 11 固定窗口统计](day11-ci-flake-statistics.md)；[Day 27 可修复候选、实现与 fork CI](day27-pr7697-e2e-flake-root-cause-analysis.md)；[Day 33 #7791 新样本与 `karmadactl top` fixture/terminal evidence split](day33-pr7791-e2e-flake-root-cause-analysis.md)；[#7795 最终 RCA 与合并证据](day33-karmadactl-top-flake-upstream-draft.md#最终合并结论)；Remedy [issue #7776](https://github.com/karmada-io/karmada/issues/7776) 已关闭，[PR #7777](https://github.com/karmada-io/karmada/pull/7777) 已合并为 `eb2e7c75ff82` | #7795 已由维护者 `/retest` 触发同 SHA v1.35 attempt 2 转绿并合并为 `1c278577e789`；无关 kind/runner 红项确认为 `E1` 非确定性，物理原因仍为 `E2` 假设，不追加 PR 代码。继续决定 migration `8ef8a08c6` 是否独立发布 |
