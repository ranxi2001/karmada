...
当前优先级：以 #7492 多组件扩缩容重调度为 Week 33 核心，等待 `RainbowMango` 明确 API 后对照 Day 43；完成周一 Descheduler 专项汇报；#7621/#7662、#7810 和 #7697 仅按新信号跟进。
...
| 主线 | 当前状态 | 下一触发条件 |
| --- | --- | --- |
| [#7492 / Day 42](internship-reports/day42-issue7492-multi-component-scale-rescheduling-intake.md) / [Day 43](internship-reports/day43-issue5115-evolution-and-7492-implementation-plan.md) | Day 42 已把 desired、accepted、applied、accounted 四份状态及 #6486 scalar precedent 梳理为独立上下文；#7492 无关联 PR，`ranxi2001` 是正式 assignee。Day 43 的 result/delta/fence 方案是待对照假设 | 等 `RainbowMango` 给出明确 API；固定官方 ref 后对照 result type/location、两版 conversion、scale/delta、failure fence、legacy、FRQ accounting 和 RB/CRB。此前不建 worktree、不发旧评论 |
...
- 2026-08-10：更新 [#7492 Day 42](internship-reports/day42-issue7492-multi-component-scale-rescheduling-intake.md) 并保留 [Day 43](internship-reports/day43-issue5115-evolution-and-7492-implementation-plan.md) 为比较基线：纠正 `mszacillo` 只有调研意向、无 formal assignment/PR；用 desired/accepted/applied/accounted 四状态解释 #5115 四期、#6486 scalar precedent、并发下发窗口和 FRQ accounting 分叉。继续等待 `RainbowMango` 明确 API；无源码 worktree 或上游动作。
...
- #7492 阻塞于 `RainbowMango` 尚未给出明确 API；Day 43 的 result-only name/replicas、mixed positive delta、current-target continuity 和 pre-cleanup Work fence 仅作对照，FRQ 的 accepted usage / desired reservation 也未定，不先实现。
...
- #7492 等 `RainbowMango` 给出明确 API；收到后固定官方 ref，对照 Day 42/43 的 result、conversion、scale/delta、failure fence、legacy、FRQ accounting 与 RB/CRB 矩阵。对照前不建 worktree、不发旧评论。
