...
| 主线 | 当前状态 | 下一触发条件 |
| --- | --- | --- |
| [#7492 PR stack](internship-reports/issue7492-pr-stack-status.md#stack-overview) | Issue 最后实质评论仍是 2026-08-20：`GetTotalBindingReplicas` 来自外部 v1.17 fork，未形成 upstream quick fix/backport 共识。#7841 current `6a51dcd9c` 已用 component-aware trigger 和 accepted-target retention 覆盖目标行为；17 checks success、GitHub `MERGEABLE` | #7835 等待 trusted user `/ok-to-test`；#7830/#7835/#7841 等待 human review，不为 fork helper 修改 upstream |
...
- 2026-08-24：复核 #7492 完整 thread、current `upstream/master@b6c92395e` 和 #7841 `6a51dcd9c`。最后讨论确认 `GetTotalBindingReplicas` 属于外部 v1.17 fork，不能作为 upstream restart bug 或 backport 证据；current #7841 在旧 scalar helper 前比较 desired/accepted components，equal 不进入 scale planner，steady reconcile 在 target 仍 eligible 时复用 accepted result，scale-up no-fit 则固定当前 target 并保留结果。#7841 当前 17 checks success、GitHub `MERGEABLE`，但 restart/no-change 尚无 live restart 证据，也没有 human design review。[最新讨论与实现影响](internship-reports/issue7492-pr-stack-status.md#最新讨论与实现影响2026-08-24)
...
- 保持 #7841 current head `6a51dcd9c` 不变；17 个 checks 已成功，不重复 push 或重跑，等待 #7830/#7835/#7841 human review。
