...
| 主线 | 当前状态 | 下一触发条件 |
| --- | --- | --- |
| [#7492 PR0-PR3 / Day 49-50](internship-reports/day50-issue7492-pr2-pr3-rebase-on-pr0.md) | #7830 head `6ff28fe4a` 的 16 个 Actions checks 与 DCO 全绿；#7833/#7835 已分别直接基于 #7837 `76589a9d5` 更新到 `98535c541`/`782232b7d`，正文逐字核对，DCO 成功且首批 11 个 Actions jobs 运行中 | 等两个 current-SHA CI 终态；不评论、不改 Draft 状态，失败时再按具体 job/log 分类 |
...
- 2026-08-16：用户确认后，#7833/#7835 已用显式 lease 分别从 `1d2ee95c4`/`b1c41a584` force-push 到 `98535c541`/`782232b7d`；GitHub head 与正文逐字核对，标题和 Draft 状态不变。DCO 成功、首批 11 个 Actions jobs 运行中，未评论，详见 [Day 50](internship-reports/day50-issue7492-pr2-pr3-rebase-on-pr0.md)。
...
- #7492 PR2/PR3：远端重排和正文更新完成，等待 `98535c541`/`782232b7d` current-SHA CI 终态。#7837 current API 仍缺旧 PR 的 list/name/replica markers，不能静默带回。
...
- #7492 等 #7833 `98535c541` 与 #7835 `782232b7d` current-SHA CI；不发评论，失败时只按具体 job/log 分类，不添加猜测性修复 commit。
