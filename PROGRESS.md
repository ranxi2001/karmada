...
| 主线 | 当前状态 | 下一触发条件 |
| --- | --- | --- |
| [PR #7830 / Day 49](internship-reports/day49-issue7492-pr1-api-compat-pr7830.md) | 已将远端 head 从 `ac32f8671` force-with-lease 更新为 `6ff28fe4a`，其 parent 为 #7837 current head `76589a9d5`；DCO 成功，upstream PR CI 已启动，未评论 | 等 current-SHA checks 终态；再按 exact-text gate 处理过期 body 描述或必要评论；#7837 merge 后仍做最终 ancestry cleanup |
...
- #7492 PR1：等待 `6ff28fe4a` upstream checks 终态。#7837 current API 仍缺旧 PR 的 list/name/replica markers，不能在后续 rebase 时静默带回。
...
- #7492 等 `6ff28fe4a` current-SHA CI；不发评论。终态后再处理 `minimal compile follow-up` 等过期 body 描述，任何 PR 文本修改走 exact-text confirmation；#7837 merge 后做最终 ancestry cleanup。
