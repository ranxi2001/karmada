...
| 主线 | 当前状态 | 下一触发条件 |
| --- | --- | --- |
| [#7492 PR stack](internship-reports/issue7492-pr-stack-status.md#stack-overview) | 最新 issue body 剩 4 项：component scale trigger、delta/scale-down estimation、失败不下发、超容量不迁移。#7835 current head 的 3 个红 job 已定性为端口冲突、artifact 上传超时和 control-plane collapse；`/retest` 已发送；#7830/#7835 仍待 review；#7841 已更新到 `6a51dcd9c` | 等待 #7835 新 run 进入 check rollup；继续等待 human review，不为环境红灯改 production code |
...
- #7492：#7830/#7835 尚无实质性 human review；#7835 `/retest` 已发送但新 run 尚未出现；#7841 已更新到当前 master 后的 `6a51dcd9c`，仍缺 current-tree live E2E、arbitrary-client admission validation、mixed-version rollout，以及 CRB、自动 target-loss failover、split-write 的 live 证据。
...
- 等待 #7835 current-head 重跑结果；不为旧环境红灯修改 planner，也不把 #7841 的集成问题塞回两个基础 PR。
