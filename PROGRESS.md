...
| 主线 | 当前状态 | 下一触发条件 |
| --- | --- | --- |
| [PR #7830 / Day 49](internship-reports/day49-issue7492-pr1-api-compat-pr7830.md) | stacked head `ac32f8671` 仅 v1.35 base E2E 红：现有 aggregated API spec 的 `BeforeEach` 创建临时 kind 集群时 `kubeadm init` 失败；PR 新增 3 个 compatibility specs 与其余 16 checks 均通过，未评论 | 经确认后只请求重跑 v1.35；不改代码，稳定后再决定 #7837 helper comment，合并后清理 stack |
...
- #7492 PR1：`ac32f8671` 需重跑无关的 v1.35 E2E 环境失败；#7837 仍缺旧 PR 的 list/name/replica markers，后续不能在 rebase 时静默带回。
...
- #7492 经用户确认后只请求重跑 `ac32f8671` 的 v1.35 failed job；不为环境失败改代码。结果稳定后再决定是否在 #7837 提供 helper commit；#7837 merge 后清理 stack，PR1 稳定后 rebase PR2。
