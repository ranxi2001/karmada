...
| 主线 | 当前状态 | 下一触发条件 |
| --- | --- | --- |
| [#7492 PR stack](internship-reports/issue7492-pr-stack-status.md#stack-overview) | #7841 公开 head `b2b27ad01` 已 Ready；16 个 checks 成功，v1.34 在不相关 rescheduling spec 的 `BeforeEach` 环境失败。live E2E 在 `d8df11c3d` 跑通 4/4；候选 `3bb0a304a` 只修注释并通过静态门禁 | 先审批是否整理并以 exact lease 更新 #7841；随后刷新 PR body/CI。rollout、admission、CRB 和自动 target-loss failover 仍是边界 |
