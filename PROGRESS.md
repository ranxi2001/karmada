...
- 2026-08-25：work-api Kubernetes/Go 升级 [PR #74](https://github.com/kubernetes-sigs/work-api/pull/74) 已 force-with-lease 更新为单一 commit `f608bdc`；L11-L16 逐项 `>=` Karmada，Gomega 也由 MVS floor `v1.40.0` 对齐到 Karmada `v1.42.0`。增量无 module 外溢，compile-only test、`make test` 与 module verification 通过；GitHub/fork exact head 一致，EasyCLA success，当时尚无人类 review。[完整记录](internship-reports/k8s-go-update.md)
