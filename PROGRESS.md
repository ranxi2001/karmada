...
- #7492：#7830/#7835 尚无实质性 human review；#7835 三个环境类红 job 尚未重跑；#7841 已更新到当前 master 后的 `6a51dcd9c`，仍缺 current-tree live E2E、arbitrary-client admission validation、mixed-version rollout，以及 CRB、自动 target-loss failover、split-write 的 live 证据。
...
- 用户确认 exact comment 后重跑 #7835 三个环境类失败；不为这些红灯修改 planner，也不把 #7841 的集成问题塞回两个基础 PR。
