...
| 主线 | 当前状态 | 下一触发条件 |
| --- | --- | --- |
| [PR #7830 / Day 49](internship-reports/day49-issue7492-pr1-api-compat-pr7830.md) | head 已从 `c0b68f728` force-with-lease 更新为 stacked `ac32f8671`；新 title/body 逐字校验，未评论。DCO 成功，其余 upstream checks 已启动 | 等 current-SHA upstream CI；有结果后再决定 #7837 helper comment，合并后清理 stack |
...
- #7492 PR1：等待 `ac32f8671` authoritative upstream CI；#7837 仍缺旧 PR 的 list/name/replica markers，后续不能在 rebase 时静默带回。
...
- #7492 等 `ac32f8671` upstream PR CI；不发评论。结果稳定后再决定是否在 #7837 提供 helper commit；#7837 merge 后清理 stack，PR1 稳定后 rebase PR2。
