...
| 主线 | 当前状态 | 下一触发条件 |
| --- | --- | --- |
| [PR #7830 / Day 49](internship-reports/day49-issue7492-pr1-api-compat-pr7830.md) | 已在 #7837 current head `76589a9d5` 上将 PR1 等价重放为本地 `6ff28fe4a`；9 文件 `+1472/-11`、DCO、race、base E2E compile、verify 均通过，远端仍为 `ac32f8671` | 用户确认 exact lease 后 force-with-lease 推送以重跑 CI；不发评论，CI 后再处理正文过期描述；#7837 merge 后仍做最终 ancestry cleanup |
...
- #7492 PR1：本地新 head 已准备；仅等待 exact force-with-lease 确认。#7837 current API 仍缺旧 PR 的 list/name/replica markers，不能在后续 rebase 时静默带回。
...
- #7492 不为 `ac32f8671` 的 v1.35 环境失败改代码；exact confirmation 后用旧远端 SHA 作 lease，把 PR head 更新为本地 `6ff28fe4a` 以重跑 CI，不发评论；CI 后再处理正文过期描述，#7837 merge 后做最终 ancestry cleanup。
