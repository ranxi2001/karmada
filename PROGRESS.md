...
| 主线 | 当前状态 | 下一触发条件 |
| --- | --- | --- |
| [#7492 PR stack](internship-reports/issue7492-pr-stack-status.md#stack-overview) | 已[回复 `@mszacillo`](https://github.com/karmada-io/karmada/issues/7492#issuecomment-5393442464)：承认 `line 51` fallthrough，同时澄清误判来自外部 v1.17 fork helper；#7841 current `6a51dcd9c` 以 component-aware trigger 和 accepted-target retention 覆盖目标行为，17 checks success、GitHub `MERGEABLE` | 等待 `@mszacillo` 确认 restart 场景或 #7841 review；#7835 等待 trusted user `/ok-to-test` |
...
- 2026-08-25：按 work-api #49/#66 的模式完成 Kubernetes/Go 依赖升级 `deps/kubernetes-1.36-go-1.26@cf1bfed`；`go.mod` L11-L16 已逐项 `>=` Karmada 对应版本。本地验证通过，[fork push CI](https://github.com/ranxi2001/work-api/actions/runs/32801974517) 的 `lint`、`verify`、`unit test`、`e2e` 全部 success。apidiff 确认 generated `SharedInformerFactory` 新增 methods 对自定义实现 source-incompatible，旧调用入口保留；PR title/body 草案已准备，未创建 upstream PR。[完整记录](internship-reports/k8s-go-update.md)
...
- #7492：#7830/#7835/#7841 尚无实质性 human review；#7835 重跑受 trusted-user `/ok-to-test` gate 阻塞；#7841 仍缺 arbitrary-client admission validation、mixed-version rollout、live restart/no-change，以及 CRB、自动 target-loss failover、split-write 的 live 证据。
...
- 保持 #7841 current head `6a51dcd9c` 不变；17 个 checks 已成功，不重复 push 或重跑，等待 `@mszacillo` 对 restart 场景的确认及 #7830/#7835/#7841 human review。
