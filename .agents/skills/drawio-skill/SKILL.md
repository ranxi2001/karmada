...
- Project data-flow, event-flow, lifecycle, or sequence diagrams whose canonical source should be `.mmd` and whose deliverable is PNG/SVG → **project-mermaid**.
- Other diagrams-as-code that live in git / render in Markdown → **project-mermaid** (Mermaid) or **plantuml** (UML).
...
| `scripts/prdiff.py` | You're setting up **automated PR diagram review** in CI — for every `.drawio` changed between two git refs it renders base/head/diff PNGs and emits a Markdown report. The upstream repository also provides a composite GitHub Action, but this runtime-only vendored copy does not install it. See `references/pr-bot.md` |
...
2. **Plan** — identify shapes, relationships, layout (LR or TB), group by tier/layer. If `.mmd` is the canonical deliverable, stop this workflow and use **project-mermaid**; the Mermaid conversion path below is only for producing a native `.drawio` deliverable.
