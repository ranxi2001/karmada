...
This Karmada workspace vendors only the skill runtime. It includes
`scripts/prdiff.py`, but it does not install the upstream repository's
composite action or example workflow. Adding either would change CI behavior
and requires a separate, explicit review.
...
- The upstream `.github/actions/drawio-diff/action.yml` — a composite action that checks
...
- The upstream `.github/workflows/drawio-pr-diff.example.yml` — a template workflow that
